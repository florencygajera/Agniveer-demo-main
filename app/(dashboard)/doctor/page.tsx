...
const INIT_RECORDS = [
  {
    ...
    hr: "64",
    weight: "72",
    notes: "Excellent fitness. No issues.",
  },
  {
    id: "MR-002",
  ...
  },
  {
    ...
    hr: "72",
    weight: "65",
    notes: "Ankle recovered. Cleared for full duty.",
  },
  {
    id: "MR-004",
    ...
    type: "Treatment",
    doctor: "Dr. Sunita Rao",
    diagnosis: "Knee pain — patellofemoral",
    status: "Under Observation",
    followup: "20 Mar 2025",
    bp: "122/80",
    hr: "76",
    weight: "70",
    notes: "Restricted from high-impact training for 3 weeks.",
  },
  {
    id: "MR-005",
  ...
  },
  {
    ...
    hr: "66",
    weight: "56",
    notes: "All vitals normal. BMI optimal.",
  },
  {
    id: "MR-007",
    ...
    hr: "62",
    weight: "74",
    notes: "Outstanding physical fitness.",
  },
  {
    id: "MR-008",
    ...
    <aside className="hidden w-56 shrink-0 flex-col border-r border-stone-200 bg-white md:flex">
      <div className="border-b border-stone-100 bg-stone-50 px-4 py-4">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-rose-600 text-sm font-bold text-white">
            S
          </div>
        ...
        <div className="mt-2">
          <Badge className="border border-rose-200 bg-rose-50 text-[10px] text-rose-600">
            Medical Officer
          </Badge>
        </div>
      </div>
...
function DashboardSection({
  ...
  const upcoming = records.filter(
    (r) => r.followup && new Date(r.followup) >= new Date()
  )
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-xs text-stone-400">
          Manage soldier health records · 14 March 2025
        </p>
      </div>
      ...
      {underObs.length > 0 && (
        <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
          <div className="flex items-center gap-2 text-sm font-bold text-amber-800">
            <AlertTriangle size={14} /> {underObs.length} Soldier
            {underObs.length > 1 ? "s" : ""} Under Observation
          </div>
          {underObs.map((r) => (
            <div
              key={r.id}
        ...
        <Card className="border-stone-200 bg-white shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Button
              ...
              className="h-6 gap-1 border-stone-200 px-2 text-[10px] text-stone-500"
            >
          ...
          <CardContent className="space-y-2 px-4 pb-4">
            {records.slice(0, 5).map((r) => (
              <div
                key={r.id}
                className="flex items-center justify-between border-b border-stone-50 py-1.5 last:border-0"
              >
                <div className="flex min-w-0 items-center gap-2.5">
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-stone-100 text-xs">
                    👤
                  </div>
                  <div className="min-w-0">
                ...
                <Badge
                  className={`ml-2 shrink-0 border text-xs ${statusStyle(r.status)}`}
                >
                  {r.status}
                </Badge>
              </div>
          ...
          <Card className="border-stone-200 bg-white shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-sm font-semibold text-stone-800">
                <CalendarDays size={13} className="text-sky-500" /> Upcoming
                Follow-ups
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 px-4 pb-4">
              {records
                .filter((r) => r.status === "Under Observation")
                .map((r) => (
              ...
              {records.filter((r) => r.status === "Under Observation")
                .length === 0 && (
                <div className="text-xs text-stone-400">
...
function AddRecordSection({
  ...
  onAdd: (r: (typeof INIT_RECORDS)[0]) => void
}) {
  const empty = {
    soldier: "",
    date: "",
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="border-stone-200 bg-white shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold text-stone-800">
            <Stethoscope size={14} className="text-rose-500" /> New Medical
            Record
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 px-4 pb-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              <Label className="text-xs font-semibold text-stone-600">
                Soldier <span className="text-rose-500">*</span>
              </Label>
              <Select
                value={form.soldier}
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Input
                  ...
                  placeholder="e.g. 72"
                  value={form.hr}
                  onChange={(e) => f("hr", e.target.value)}
                  className="h-8 text-sm"
                />
              </div>
              <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              placeholder="Primary diagnosis or finding…"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Textarea
              rows={3}
              placeholder="Additional notes, treatment prescribed, observations…"
              value={
                form.notes.startsWith("Fit") ||
                form.notes.startsWith("Light") ||
                form.notes.startsWith("Rest") ||
                form.notes.startsWith("Temp") ||
                form.notes.startsWith("Ref")
                  ? ""
                  : form.notes
              }
          ...
          <div className="flex items-center gap-3 pt-1">
            <Button
              onClick={handleSave}
              className="gap-2 bg-rose-600 text-sm text-white hover:bg-rose-700"
            >
            ...
            <Button
              ...
              className="gap-2 text-sm text-stone-500"
            >
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-xs text-stone-400">
          All soldier health records · Confidential
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={statusF} onValueChange={setStatusF}>
          <SelectTrigger className="h-8 w-40 border-stone-200 bg-white text-xs">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            ...
            <tbody className="divide-y divide-stone-50">
              {filtered.length === 0 ? (
                ...
                filtered.map((r) => (
                  <tr
                    key={r.id}
                    className={`hover:bg-stone-50 ${r.status === "Under Observation" ? "bg-amber-50/30" : ""}`}
                  >
                    ...
                    <td className="px-3 py-3">
                      <Badge className={`border text-xs ${typeColor(r.type)}`}>
                        {r.type}
                      </Badge>
                    </td>
                    <td className="px-3 py-3 text-xs whitespace-nowrap text-stone-600">
                      {r.doctor}
                    </td>
                    <td className="max-w-[150px] px-3 py-3 text-xs text-stone-700">
                      <span className="line-clamp-2">{r.diagnosis}</span>
                    </td>
                    ...
                    <td className="px-3 py-3 text-xs whitespace-nowrap text-stone-400">
                      {r.followup || "—"}
                    </td>
                    <td className="px-3 py-3">
                      <Badge
                        className={`border text-xs ${statusStyle(r.status)}`}
                      >
                        {r.status}
                      </Badge>
                    </td>
                    <td className="max-w-[120px] px-3 py-3 text-xs text-stone-400">
...
function StatsSection({ records }: { records: typeof INIT_RECORDS }) {
  ...
  const underObs = records.filter(
    (r) => r.status === "Under Observation"
  ).length
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-xs text-stone-400">
          Overall fitness overview for the command
        </p>
      </div>
          ...
          >
            <CardContent className="px-4 pt-3 pb-3">
              ...
              <div className="mt-0.5 text-[10px] text-stone-400">
                {Math.round((c.value / total) * 100)}% of total
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card className="border-stone-200 bg-white shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-sm font-semibold text-stone-800">
              <Activity size={13} className="text-emerald-500" /> Fitness Status
              Breakdown
            </CardTitle>
          </CardHeader>
        ...
        <Card className="border-stone-200 bg-white shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-sm font-semibold text-stone-800">
              <Stethoscope size={13} className="text-rose-500" /> Record Type
              Distribution
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 px-4 pb-4">
            {Object.entries(types).map(([type, count]) => (
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  <Badge
                    className={`shrink-0 border text-xs ${typeColor(type)}`}
                  >
                    {type}
                  </Badge>
                </div>
                <div className="flex items-center gap-2">
                  <div className="h-2 w-24 overflow-hidden rounded-full bg-stone-100">
                    <div
                      className="h-full rounded-full bg-[#1a2d4a]"
                      style={{ width: `${(count / total) * 100}%` }}
                    />
                  </div>
                  <span className="w-4 text-right text-sm font-black text-stone-700">
                    {count}
                  </span>
                </div>
              </div>
      ...
      <Card className="border-stone-200 bg-white shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-sm font-semibold text-stone-800">
            <HeartPulse size={13} className="text-rose-500" /> Average Vitals
            Across Command
          </CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              ...
              <div
                key={v.label}
                className="rounded-lg border border-stone-100 bg-stone-50 p-4 text-center"
              >
                <div className={`mb-2 flex justify-center ${v.color}`}>
                  {v.icon}
                </div>
                <div className={`text-xl font-black ${v.color}`}>{v.value}</div>
                <div className="mt-0.5 text-[10px] font-semibold text-stone-500">
                  {v.label}
                </div>
                <div className="mt-0.5 text-[9px] text-stone-400">{v.sub}</div>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 border-b border-stone-200 bg-white/80 backdrop-blur-sm">
          <div className="flex items-center justify-between px-4 py-3 sm:px-6">
            ...
            <Badge className="shrink-0 border border-rose-200 bg-rose-50 text-xs text-rose-600">
              Medical Officer
            </Badge>
          </div>
