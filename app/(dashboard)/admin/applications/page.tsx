...
const INIT_APPS: Application[] = [
  ...
  },
  {
    ...
    status: "Verified",
    notes: "All documents verified. Eligible.",
    phone: "9898989892",
  ...
  },
  {
    ...
    status: "Rejected",
    notes:
      "Education qualification not meeting minimum criteria for applied trade.",
    phone: "9898989894",
  ...
  },
  {
    ...
    verifiedBy: "Maj. Ankit Verma",
    status: "Selected",
    notes: "All checks passed. Eligible for training.",
    phone: "9898989895",
  ...
  },
  {
    ...
    height: "163",
    weight: "54",
    trade: "Agniveer Clerk",
    appliedOn: "04 Mar 2025",
    verifiedBy: "—",
    status: "Pending Docs",
    notes: "Missing domicile certificate.",
    phone: "9898989898",
...
function StatusBadge({ status }: { status: AppStatus }) {
  ...
  return (
    <Badge
      className={`gap-1 border text-xs font-medium whitespace-nowrap ${c.pill}`}
    >
      {c.icon}
      {status}
    </Badge>
...
function sc(v: number) {
  ...
  if (v >= 75) return "text-[#4a5c2f]"
  if (v >= 50) return "text-amber-600"
  return "text-rose-500"
}
function bc(v: number) {
  if (v >= 90) return "bg-emerald-500"
  if (v >= 75) return "bg-[#4a5c2f]"
  if (v >= 50) return "bg-amber-500"
  return "bg-rose-500"
}

// ── Grade: Excellent / Good / SAT / Fail ──────────────────────
function grade(v: number) {
  ...
  if (v >= 75) return { l: "Good", c: "bg-sky-100 text-sky-700 border-sky-200" }
  if (v >= 50)
    return { l: "SAT", c: "bg-amber-100 text-amber-700 border-amber-200" }
  return { l: "Fail", c: "bg-rose-100 text-rose-600 border-rose-200" }
}
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] gap-0 overflow-y-auto p-0 sm:max-w-2xl">
        <div className="flex items-start justify-between gap-4 rounded-t-xl bg-[#1a2d4a] px-5 py-4">
          <div>
            <div className="mb-1 text-xs text-white/60">Application Detail</div>
            <h2 className="text-lg font-bold text-white">{app.name}</h2>
            <div className="mt-1.5 flex items-center gap-2">
              <span className="rounded bg-white/10 px-2 py-0.5 font-mono text-[11px] text-orange-300">
                {app.id}
              </span>
            ...
            <div className="overflow-hidden rounded-lg border border-stone-200">
              ...
              {[
                ...
                >
                  ...
                  <span className="max-w-[55%] truncate text-right text-xs font-medium text-stone-700">
                    {v}
                  </span>
                </div>
              ))}
          ...
          <div className="overflow-hidden rounded-lg border border-stone-200">
            ...
            <div className="space-y-3 p-4">
              <div className="flex items-center gap-3">
                <span className="w-24 text-xs text-stone-400">Status</span>
                <StatusBadge status={app.status} />
              </div>
              <div className="flex items-center gap-3">
                <span className="w-24 text-xs text-stone-400">Verified By</span>
                <span className="text-xs font-medium text-stone-700">
                  {app.verifiedBy}
                </span>
              </div>
              <div className="flex items-start gap-3">
                <span className="mt-0.5 w-24 text-xs text-stone-400">
                  Notes
                </span>
                <span className="flex-1 text-xs text-stone-600">
                  {app.notes === "—" ? "No notes added." : app.notes}
                </span>
              </div>
            </div>
          </div>
        ...
        <DialogFooter className="-mx-0 -mb-0 flex flex-row gap-2 rounded-b-xl border-t bg-stone-50 p-4">
          ...
          <Button
            variant="outline"
            onClick={onEdit}
            className="gap-1.5 border-[#1a2d4a] text-sm text-[#1a2d4a] hover:bg-[#1a2d4a] hover:text-white"
          >
            <Pencil size={13} /> Edit
          </Button>
          <Button
            onClick={onDelete}
            className="gap-1.5 bg-rose-600 text-sm text-white hover:bg-rose-700"
          >
      ...
      <DialogContent className="max-h-[90vh] gap-0 overflow-y-auto p-0 sm:max-w-2xl">
        <DialogHeader className="px-5 pt-5 pb-0">
          <DialogTitle className="flex items-center gap-2 text-base text-stone-800">
            {mode === "create" ? (
              <Plus size={15} className="text-[#4a5c2f]" />
            ) : (
              <Pencil size={15} className="text-[#1a2d4a]" />
            )}
            {mode === "create"
              ? "Add New Application"
              : `Edit Application — ${initial?.id}`}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-5 p-5">
          <div>
            <div className="mb-3 text-xs font-bold tracking-wide text-stone-400 uppercase">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <F label="Medical Condition">
                <Select
                  value={form.medical}
                  onValueChange={(v) => set("medical", v)}
                >
                  <SelectTrigger className="h-8 text-sm">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {["None", "Minor - Declared", "Under Observation"].map(
                      (m) => (
              ...
              <F label="Applied On">
                <Input
                  type="date"
                  value={form.appliedOn.includes("-") ? form.appliedOn : ""}
        ...
        <DialogFooter className="-mx-0 -mb-0 flex flex-row gap-2 rounded-b-xl border-t bg-stone-50 p-4">
          ...
          <Button
            onClick={handleSave}
            className="gap-1.5 bg-[#4a5c2f] text-sm text-white hover:bg-[#344228]"
          >
            <Save size={13} />
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="gap-0 p-0 sm:max-w-sm">
        ...
        <DialogFooter className="-mx-0 -mb-0 flex flex-row gap-2 rounded-b-xl border-t bg-stone-50 p-4">
          ...
          <Button
            onClick={onConfirm}
            className="flex-1 gap-1.5 bg-rose-600 text-sm text-white hover:bg-rose-700"
          >
            <Trash2 size={13} /> Delete
          </Button>
        </DialogFooter>
...
// ═══════════════════════════════════════════════════════════════
// Agniveer FORM MODAL — discipline hidden from UI, kept in data
// ═══════════════════════════════════════════════════════════════
...
function AgniveerFormModal({
  ...
  const set = (k: keyof typeof form, v: string | number) =>
    setForm((f) => ({ ...f, [k]: v }))
  // Discipline excluded from overall calc
  const calcOverall = (f: typeof form) =>
    Math.round(
      ((f.physical + f.weapons + f.mental + f.combat + f.attendance) / 5) * 10
    ) / 10
  const F = ({
    label,
    req,
      ...
      <DialogContent className="max-h-[90vh] gap-0 overflow-y-auto p-0 sm:max-w-2xl">
        <DialogHeader className="px-5 pt-5 pb-0">
          <DialogTitle className="flex items-center gap-2 text-base text-stone-800">
            ...
            {mode === "create"
              ? "Enrol New Agniveer"
              : `Edit Agniveer — ${initial?.AgniveerId}`}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-5 p-5">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <F label="Agniveer ID" req>
                <Input
                  value={form.AgniveerId}
                  onChange={(e) => set("AgniveerId", e.target.value)}
              ...
              <F label="Battalion">
                <Select
                  ...
                  onValueChange={(v) => set("battalion", v)}
                >
                  <SelectTrigger className="h-8 text-sm">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <F label="Status">
                <Select
                  value={form.status}
                  onValueChange={(v) => set("status", v as Agniveer["status"])}
                >
                  <SelectTrigger className="h-8 text-sm">
                    <SelectValue />
                  </SelectTrigger>
          ...
          {/* Scores — Discipline removed from UI */}
          <div>
            <div className="mb-3 text-xs font-bold tracking-wide text-stone-400 uppercase">
            ...
            <div className="mt-3 flex items-center gap-3 rounded-lg border border-stone-200 bg-stone-50 px-4 py-2.5">
              ...
              <span className={`text-xl font-black ${sc(form.overall)}`}>
                {form.overall}
              </span>
              <Badge className={`border text-xs ${grade(form.overall).c}`}>
                {grade(form.overall).l}
              </Badge>
              <button
                type="button"
                onClick={() =>
                  setForm((f) => ({ ...f, overall: calcOverall(f) }))
                }
        ...
        <DialogFooter className="-mx-0 -mb-0 flex flex-row gap-2 rounded-b-xl border-t bg-stone-50 p-4">
          ...
          <Button
            onClick={() =>
              onSave({ id: initial?.id ?? String(Date.now()), ...form })
            }
            className="gap-1.5 bg-[#4a5c2f] text-sm text-white hover:bg-[#344228]"
          >
            <Save size={13} />
            {mode === "create" ? "Enrol Agniveer" : "Save Changes"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
// ═══════════════════════════════════════════════════════════════
// Agniveer DETAIL MODAL — discipline removed from scores
// ═══════════════════════════════════════════════════════════════
function AgniveerDetailModal({
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] gap-0 overflow-y-auto p-0 sm:max-w-2xl">
        <div className="flex items-start justify-between rounded-t-xl bg-[#1a2d4a] px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap gap-1.5">
                <span className="rounded bg-white/10 px-1.5 py-0.5 font-mono text-[10px] text-orange-300">
                  {Agniveer.AgniveerId}
                </span>
          ...
          <div className="shrink-0 text-right">
            <div
              className={`text-3xl font-black ${Agniveer.overall >= 90 ? "text-emerald-400" : Agniveer.overall >= 75 ? "text-amber-300" : "text-orange-400"}`}
            >
              {Agniveer.overall}
            </div>
            <div className="text-[10px] text-white/50">
              Overall · Global #{globalRank}
            </div>
          </div>
        </div>
        <div className="space-y-4 p-5">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {scores.map((s) => (
              ...
              >
                ...
                <div className={`text-xl font-black ${sc(Agniveer[s.key])}`}>
                  {Agniveer[s.key]}
                </div>
                <Badge
                  className={`mt-1 border text-[10px] ${grade(Agniveer[s.key]).c}`}
                >
                  {grade(Agniveer[s.key]).l}
                </Badge>
                <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-stone-200">
                  <div
                    className={`h-full rounded-full ${bc(Agniveer[s.key])}`}
                    style={{ width: `${Agniveer[s.key]}%` }}
                  />
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="overflow-hidden rounded-lg border border-stone-200">
              ...
              {[
                ...
                ["City", Agniveer.city],
                ["Blood", Agniveer.blood],
                ["Phone", Agniveer.phone],
                ["Email", Agniveer.email],
              ].map(([k, v]) => (
                <div
                  key={k}
                  className="flex justify-between border-b border-stone-50 px-4 py-2 last:border-0"
                >
                  <span className="text-xs text-stone-400">{k}</span>
                  <span className="max-w-[60%] truncate text-xs font-medium text-stone-700">
                    {v}
                  </span>
                </div>
              ))}
            ...
            <div className="overflow-hidden rounded-lg border border-stone-200">
              ...
              {[
                ["Rank", Agniveer.rank],
                ["Battalion", Agniveer.battalion],
                ["Joined", Agniveer.joining],
                ["Medical", Agniveer.medical],
                ["Status", Agniveer.status],
              ].map(([k, v]) => (
                <div
                  key={k}
                  className="flex justify-between border-b border-stone-50 px-4 py-2 last:border-0"
...
function Toast({ msg, type }: { msg: string; type: "success" | "error" }) {
  return (
    <div
      className={`fixed right-5 bottom-5 z-[100] flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium shadow-lg ${type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-700"}`}
    >
      {type === "success" ? (
...
export default function RecruitmentPage() {
  ...
  const saveApp = (updated: Application) => {
    if (apps.find((a) => a.id === updated.id)) {
  ...
  const bulkDelete = () => {
    ...
    showToast(`${selectedApps.size} deleted.`)
    setSelectedApps(new Set())
  }
  const bulkStatus = (status: AppStatus) => {
    setApps((prev) =>
      prev.map((a) =>
        selectedApps.has(a.id) ? { ...a, status, verifiedBy: adminName } : a
      )
    )
    showToast(`${selectedApps.size} marked ${status}.`)
    setSelectedApps(new Set())
  }
  ...
  const saveAgniveer = (updated: Agniveer) => {
    if (Agniveer.find((s) => s.id === updated.id)) {
      setAgniveer((prev) =>
        prev.map((s) => (s.id === updated.id ? updated : s))
      )
      showToast("Agniveer updated.")
    } else {
      setAgniveer((prev) => [updated, ...prev])
      showToast("Agniveer enrolled.")
    }
    setEditSol(null)
    setCreateSol(false)
    setViewSol(null)
  ...
  const filteredApps = apps.filter((a) => {
    ...
    return (
      matchTab &&
      (!q ||
        [a.name, a.id, a.state, a.trade].some((v) =>
  ...
  const filteredSols = Agniveer.filter((s) => {
    ...
    const matchQ =
      ...
      s.AgniveerId.toLowerCase().includes(q) ||
      s.battalion.toLowerCase().includes(q)
    const matchF =
      solFilter === "all" ||
      (solFilter === "active" && s.status === "active") ||
      (solFilter === "leave" && s.status === "on_leave") ||
      (solFilter === "good" && s.overall >= 75) ||
      (solFilter === "attn" && s.overall < 50)
    return matchQ && matchF
  })

  const TAB_ACTIVE: Record<FilterTab, string> = {
    All: "bg-stone-800 text-white",
    "Under Review": "bg-sky-600 text-white",
    Verified: "bg-emerald-600 text-white",
    "Pending Docs": "bg-amber-500 text-white",
    Selected: "bg-[#4a5c2f] text-white",
    ...
    <div className="min-h-screen bg-[#f4f3ef] font-sans">
      {viewApp && (
        <AppDetailModal
          ...
          onDelete={() => setDeleteApp(viewApp)}
        />
      )}
      {(editApp || createApp) && (
        <AppFormModal
          initial={editApp ?? undefined}
          mode={editApp ? "edit" : "create"}
          onSave={saveApp}
          onClose={() => {
            setEditApp(null)
            setCreateApp(false)
      ...
      {viewSol && (
        <AgniveerDetailModal
          ...
          onDelete={() => setDeleteSol(viewSol)}
        />
      )}
      {(editSol || createSol) && (
        <AgniveerFormModal
          initial={editSol ?? undefined}
          mode={editSol ? "edit" : "create"}
          onSave={saveAgniveer}
          onClose={() => {
            setEditSol(null)
            setCreateSol(false)
      ...
      {deleteSol && (
        <DeleteConfirm
          name={deleteSol.name}
          id={deleteSol.AgniveerId}
          onConfirm={confirmDeleteSol}
          onClose={() => setDeleteSol(null)}
        />
      )}
        ...
        <div className="mx-auto flex max-w-screen-2xl flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          <div>
            ...
            <p className="mt-0.5 text-xs text-stone-500">
              Full CRUD — Create, Read, Update, Delete applications and Agniveer
              records
            </p>
          </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              {[
                ...
                {
                  label: "Under Review",
                  value: apps.filter((a) => a.status === "Under Review").length,
                  color: "text-sky-600",
                  icon: <Clock size={14} />,
                },
                {
                  label: "Verified",
                  value: apps.filter((a) => a.status === "Verified").length,
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <div className="scrollbar-hide flex flex-1 gap-1.5 overflow-x-auto pb-1">
                {FILTER_TABS.map((tab) => (
                  <button
                    ...
                    className={`shrink-0 rounded-lg border px-3 py-1.5 text-xs font-semibold transition-all ${activeTab === tab ? TAB_ACTIVE[tab] : "border-stone-200 bg-white text-stone-500 hover:border-stone-300"}`}
                  >
                    {tab} ({countFor(tab)})
                  </button>
                ))}
              ...
              <div className="flex shrink-0 gap-2">
                ...
                <Button
                  ...
                  className="h-8 shrink-0 gap-1.5 bg-[#4a5c2f] text-xs text-white hover:bg-[#344228]"
                >
            ...
            {selectedApps.size > 0 && (
              <div className="flex flex-wrap items-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-4 py-2.5">
                ...
                <div className="ml-2 flex flex-wrap gap-1.5">
                  ...
                  <button
                    onClick={bulkDelete}
                    className="rounded-md border border-rose-200 bg-rose-100 px-2.5 py-1 text-[11px] font-semibold text-rose-600"
                  >
                    Delete All
                  </button>
                </div>
            ...
            <Card className="overflow-hidden border-stone-200 bg-white shadow-sm">
                    ...
                    <tr className="border-b border-stone-100 bg-stone-50">
                      <th className="w-8 px-3 py-2.5">
                        <input
                          ...
                          onChange={() =>
                            toggleAll(filteredApps.map((a) => a.id))
                          }
                  ...
                  <tbody className="divide-y divide-stone-50">
                    {filteredApps.length === 0 ? (
                      ...
                      filteredApps.map((app) => (
                        <tr
                          key={app.id}
                          className={`transition-colors ${selectedApps.has(app.id) ? "bg-sky-50" : app.status === "Rejected" ? "bg-rose-50/20 hover:bg-rose-50/40" : app.status === "Selected" ? "bg-emerald-50/20 hover:bg-emerald-50/40" : "hover:bg-stone-50"}`}
                        >
                          ...
                          <td className="px-3 py-2.5 font-semibold whitespace-nowrap text-stone-800">
                            {app.name}
                          </td>
                          <td className="px-3 py-2.5 text-xs text-stone-600">
                            {app.state}
                          </td>
                          <td className="px-3 py-2.5 text-xs whitespace-nowrap text-stone-600">
                            {app.education}
                          </td>
                          <td className="px-3 py-2.5 text-xs text-stone-600">
                            {app.height} cm
                          ...
                          <td className="px-3 py-2.5 text-xs whitespace-nowrap text-stone-400">
                            {app.appliedOn}
                          </td>
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="flex h-6 w-6 items-center justify-center rounded border border-stone-200 bg-white text-stone-500 transition-colors hover:bg-stone-100"
                              >
                                <Eye size={11} />
                              </button>
                              <button
                                onClick={() => setEditApp(app)}
                                className="flex h-6 w-6 items-center justify-center rounded border border-[#1a2d4a]/20 bg-white text-[#1a2d4a] transition-colors hover:bg-[#1a2d4a] hover:text-white"
                              >
                                <Pencil size={11} />
                              </button>
                              <button
                                onClick={() => setDeleteApp(app)}
                                className="flex h-6 w-6 items-center justify-center rounded border border-rose-200 bg-white text-rose-500 transition-colors hover:bg-rose-600 hover:text-white"
                              >
                                <Trash2 size={11} />
                              </button>
                            </div>
              ...
              <div className="flex items-center justify-between border-t border-stone-100 bg-stone-50/50 px-4 py-2.5">
                <p className="text-xs text-stone-400">
                  {filteredApps.length} of {apps.length} applications
                </p>
                <p className="text-xs text-stone-400">
                  👁 View · ✏️ Edit · 🗑 Delete · ☑ Select for bulk actions
                </p>
              </div>
            </Card>
          </>
        ...
        {section === "Agniveer" && (
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                {
                  label: "Total Agniveer",
                  value: Agniveer.length,
                  color: "text-[#1a2d4a]",
                },
                {
                  label: "Active",
                  value: Agniveer.filter((s) => s.status === "active").length,
                  color: "text-emerald-600",
                },
                {
                  label: "On Leave",
                  value: Agniveer.filter((s) => s.status === "on_leave").length,
                ...
                },
                {
                  ...
                  value:
                    Math.round(
                      (Agniveer.reduce((a, s) => a + s.overall, 0) /
                        Agniveer.length) *
                        10
                    ) / 10,
                  color: "text-[#4a5c2f]",
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-stone-400">
                {filteredSols.length} Agniveer
              </span>
              <Button
                onClick={() => setCreateSol(true)}
                className="ml-auto h-8 shrink-0 gap-1.5 bg-[#4a5c2f] text-xs text-white hover:bg-[#344228]"
              >
            ...
            <Card className="overflow-hidden border-stone-200 bg-white shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-stone-100 bg-stone-50">
                      {/* Rank and Discipline columns removed */}
                      {[
                        "Agniveer ID",
                        "Name",
                        "Battalion",
                  ...
                  <tbody className="divide-y divide-stone-50">
                    {filteredSols.length === 0 ? (
                      ...
                      filteredSols.map((s) => (
                        <tr
                          key={s.id}
                          className={`transition-colors hover:bg-[#f0f5e8] ${s.overall >= 90 ? "bg-emerald-50/20" : s.overall < 50 ? "bg-rose-50/20" : ""}`}
                        >
                          <td className="px-3 py-2.5 font-mono text-xs whitespace-nowrap text-stone-400">
                            {s.AgniveerId}
                          </td>
                          ...
                          {/* Rank cell removed */}
                          <td className="px-3 py-2.5">
                            <Badge className="border border-stone-200 bg-stone-50 font-mono text-[10px] text-stone-600">
                              {s.battalion}
                            </Badge>
                          </td>
                          {/* Discipline cell removed; 5 score columns only */}
                          {(
                            [
                              "physical",
                              "weapons",
                            ...
                            <td key={k} className="px-3 py-2.5">
                              <div className="flex items-center gap-1">
                                ...
                                <span
                                  className={`text-xs font-bold ${sc(s[k])}`}
                                >
                                  {s[k]}
                                </span>
                              </div>
                          ...
                          <td className="px-3 py-2.5">
                            <span
                              className={`text-base font-black ${sc(s.overall)}`}
                            >
                              {s.overall}
                            </span>
                          </td>
                          ...
                          <td className="px-3 py-2.5">
                            <Badge
                              className={`border text-xs ${grade(s.overall).c}`}
                            >
                              {grade(s.overall).l}
                            </Badge>
                          </td>
                          <td className="px-3 py-2.5">
                            <Badge
                              className={`border text-xs ${s.status === "active" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : s.status === "on_leave" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-stone-200 bg-stone-50 text-stone-500"}`}
                            >
                              {s.status === "active"
                                ? "Active"
                                : s.status === "on_leave"
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex items-center gap-1">
                              <button
                                onClick={() => setViewSol(s)}
                                className="flex h-6 w-6 items-center justify-center rounded border border-stone-200 bg-white text-stone-500 transition-colors hover:bg-stone-100"
                              >
                                <Eye size={11} />
                              </button>
                              <button
                                onClick={() => setEditSol(s)}
                                className="flex h-6 w-6 items-center justify-center rounded border border-[#1a2d4a]/20 bg-white text-[#1a2d4a] transition-colors hover:bg-[#1a2d4a] hover:text-white"
                              >
                                <Pencil size={11} />
                              </button>
                              <button
                                onClick={() => setDeleteSol(s)}
                                className="flex h-6 w-6 items-center justify-center rounded border border-rose-200 bg-white text-rose-500 transition-colors hover:bg-rose-600 hover:text-white"
                              >
                                <Trash2 size={11} />
                              </button>
              ...
              <div className="flex items-center justify-between border-t border-stone-100 bg-stone-50/50 px-4 py-2.5">
                <p className="text-xs text-stone-400">
                  {filteredSols.length} of {Agniveer.length} Agniveer
                </p>
                <p className="text-xs text-stone-400">
                  👁 View · ✏️ Edit · 🗑 Delete
                </p>
