...
function GradeBadge({ grade }: { grade: Grade }) {
  ...
  return (
    <Badge className={`border text-xs font-semibold ${map[grade]}`}>
      {grade}
    </Badge>
  )
}
...
function StatusBadge({ status }: { status: AgniveerStatus }) {
  ...
  return (
    <Badge className={`border text-xs font-medium ${map[status]}`}>
      {labels[status]}
    </Badge>
  )
}
...
function MiniBar({ value }: { value: number }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        className={`text-xs font-semibold tabular-nums ${scoreColor(value)}`}
      >
        {value}
      </span>
    </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] gap-0 overflow-y-auto p-0 sm:max-w-2xl">
        {/* Header */}
        <div className="rounded-t-xl bg-rose-600 px-5 py-4 text-white">
        ...
        <div className="space-y-5 p-5">
              ...
              {weakAreas.map((area) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    {area.key === "physical" && (
                      <Dumbbell size={14} className="text-rose-600" />
                    )}
                    {area.key === "weapons" && (
          ...
          <div className="flex gap-2 pt-2">
            <Button
              className="flex-1 bg-rose-600 text-xs hover:bg-rose-700"
              onClick={onClose}
            >
              Acknowledge
            </Button>
...
function AgniveerDetailModal({
  ...
  const [tab, setTab] = useState<
    "overview" | "scores" | "personal" | "equipment" | "events"
  >("overview")
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] gap-0 overflow-y-auto p-0 sm:max-w-3xl">
        {/* ── Header strip ── */}
        <div className="rounded-t-xl bg-[#1a2d4a] px-5 py-4 text-white">
        ...
        <div className="space-y-4 p-5">
            ...
            <>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {scores.map((sc) => (
                  ...
                  >
                    <div className="mb-1.5 flex items-center gap-1 text-[10px] text-stone-400">
                      {sc.icon}
                      {sc.label}
                    </div>
                    <div
                      className={`text-2xl font-black ${scoreColor(sc.value)}`}
                    >
                      {sc.value}
                    </div>
                    <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-stone-200">
                      <div
                        className={`h-full rounded-full ${barColor(sc.value)}`}
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
                    ...
                    {scores
                      ...
                      .map((s) => (
                        <div
                          key={s.key}
                          className="py-0.5 text-xs text-emerald-700"
                        >
                ...
                {scores.filter((s) => s.value >= 70 && s.value < 85).length >
                  0 && (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                    ...
                    {scores
                      .filter((s) => s.value >= 70 && s.value < 85)
                      .map((s) => (
                        <div
                          key={s.key}
                  ...
                  <div className="rounded-lg border border-rose-200 bg-rose-50 p-3">
                    ...
                    {scores
                      ...
                      .map((s) => (
                        <div
                          key={s.key}
                          className="py-0.5 text-xs text-rose-700"
                        >
                          {s.label} — <strong>{s.value}</strong>
                        </div>
                      ))}
                ...
                <tbody className="divide-y divide-stone-50">
                  {scores.map((sc) => (
                    <tr key={sc.key} className="hover:bg-stone-50">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-lg font-black ${scoreColor(sc.value)}`}
                        >
                          {sc.value}
                        </span>
          ...
          {tab === "personal" && (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="overflow-hidden rounded-lg border border-stone-200">
                <div className="border-b border-stone-100 bg-stone-50 px-4 py-2 text-sm font-semibold text-stone-800">
                  ...
                  {Agniveer.equipment.map((eq, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-stone-50">
                        ...
                        <td className="px-4 py-2.5 text-sm font-medium text-stone-700">
                          📦 {eq}
                        </td>
      ...
      >
        <CardContent className="px-5 py-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex gap-5">
              {[
                ...
                >
                  ...
                  <span className="text-center text-[9px] font-semibold tracking-wide text-stone-400 uppercase">
                    {stat.label}
                  </span>
                </div>
              ))}
            ...
            <div className="shrink-0 text-right">
              <div
                className={`text-3xl font-black ${scoreColor(battalion.score)}`}
              >
                {battalion.score}
              </div>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 font-semibold whitespace-nowrap text-[#1a2d4a] hover:underline">
                      {s.name}
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <span
                        className={`text-lg font-black ${scoreColor(s.overall)}`}
                      >
                        {s.overall}
                      </span>
                    </td>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {BATTALIONS.map((bat) => (
            <button
              key={bat.id}
              onClick={() => router.replace(`/admin/battalions?bat=${bat?.id}`)}
              className={`group w-full rounded-xl border border-l-4 border-stone-200 text-left ${bat.accentColor} bg-white p-4 shadow-sm transition-all hover:border-stone-300 hover:shadow-md`}
            >
              {/* Top row */}
              <div className="mb-3 flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="text-sm leading-tight font-bold text-stone-900 transition-colors group-hover:text-[#1a2d4a]">
                    {bat.name}
                  </div>
                  <div className="mt-0.5 font-mono text-[10px] font-semibold text-stone-400">
                    {bat.code}
                  </div>
                  <div className="mt-0.5 flex items-center gap-0.5 text-[10px] text-stone-400">
                    <MapPin size={8} />
                    {bat.location}
                  </div>
                ...
                <div className="shrink-0 text-right">
                  <div
                    className={`text-2xl font-black ${scoreColor(bat.score)}`}
                  >
                    {bat.score}
                  </div>
              ...
              <div className="mb-3 grid grid-cols-4 gap-1 rounded-lg border border-stone-100 bg-stone-50 px-2 py-2">
                {[
                  ...
                  { label: "Active", value: bat.active },
                  { label: "Leave", value: bat.onLeave },
                  { label: "≥85", value: bat.scoreAbove85 },
                ].map((s) => (
              ...
              <div className="space-y-1.5">
                {[
                  ["Physical", bat.physical],
                  ["Weapons", bat.weapons],
                  ["Mental", bat.mental],
                ].map(([l, v]) => (
                  <div key={l as string} className="flex items-center gap-2">
                    <span className="w-12 shrink-0 text-[10px] text-stone-400">
                      {l}
                    </span>
                    <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-stone-100">
                      <div
                        className={`h-full rounded-full ${barColor(v as number)}`}
                        style={{ width: `${v}%` }}
                      />
                    </div>
                    <span
                      className={`w-6 text-right text-[10px] font-bold ${scoreColor(v as number)}`}
                    >
                      {v}
                    </span>
                  </div>
                ...
                <div key={s.id} className="flex items-center gap-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-stone-400">
                      {s.battalion} · {s.battalionName}
                    </div>
                  </div>
                  <GradeBadge grade={s.grade} />
                  <span
                    className={`shrink-0 text-sm font-black ${scoreColor(s.overall)}`}
                  >
                    {s.overall}
                  </span>
                </div>
        ...
        <Card className="border-stone-200 bg-white shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="flex items-center gap-2 text-base font-semibold text-stone-800">
              <AlertTriangle size={14} className="text-amber-500" /> Needs
              Attention
            </CardTitle>
            ...
            {needsAttention.length === 0 ? (
              ...
              <div className="divide-y divide-stone-50">
                {needsAttention.map((s) => (
                  <div
                    key={s.id}
                    className="-mx-2 flex cursor-pointer items-center gap-3 rounded-lg px-2 py-2.5 transition-colors hover:bg-stone-50"
                  ...
                  >
                    ...
                    <Badge
                      className={`shrink-0 border text-xs ${s.overall < 70 ? "border-rose-300 bg-rose-100 text-rose-700" : "border-amber-200 bg-amber-50 text-amber-700"}`}
                    >
